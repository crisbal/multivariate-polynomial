...
(defun update-test (&key category result)
  (cond ((eql category 'monomial-parsing)
     (if result
         (defparameter *monomial-parsing-ok*
    ...
    ((eql category 'polynomial-parsing)
     (if result
         (defparameter *polynomial-parsing-ok*
    ...
    ((eql category 'utility-functions)
     (if result
         (defparameter *utility-functions-ok*
    ...
    ((eql category 'arithmetic-functions)
     (if result
         (defparameter *arithmetic-functions-ok*
...
(defun test-polyminus ()
  (test-function :result '(POLY ((M -4 0 NIL)
                 (M -1 1 ((V 1 X)))))
         :input1 '(+ -1 (* x x y))
         :input2 '(+ 3 (* x) (* x x y))
         :function #'polyminus
...
(format t
    "~&~%Tests result, passed: ~d, failed: ~d"
...
(format t
    "~&Monomial parsing tests, passed: ~d, failed: ~d"
    *monomial-parsing-ok*
    *monomial-parsing-errors*)

(format t
    "~&Polynomial parsing tests, passed: ~d, failed: ~d"
    *polynomial-parsing-ok*
    *polynomial-parsing-errors*)

(format t
    "~&Utility functions tests, passed: ~d, failed: ~d"
    *utility-functions-ok*
    *utility-functions-errors*)

(format t
    "~&Arithmetic functions tests, passed: ~d, failed: ~d"
